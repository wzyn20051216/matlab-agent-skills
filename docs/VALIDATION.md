...
本仓库是开源的，但要激活完整工作流能力，默认前提是具备已授权的 MATLAB R2026a 正版环境，以及任务所需的 Simulink、Coder、Embedded Coder、硬件支持包和相关第三方工具链。

如果缺少正版许可或相应工具箱，仓库中的部分脚本仍可运行为探测或降级验证模式，但不能把这种“部分通过”误判为完整闭环能力已经具备。
